...
/*   Updated: 2019/03/11 16:00:59 by cuzureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void		draw_map_loop(t_wolf *wolf, t_m *m)
{
	while (m->x < m->max_x + 1)
	{
		...
		|| wolf->map[(unsigned int)m->ry][(unsigned int)m->rx].win == 1))
		{
			...
			ft_draw_rectangle(m->point_a, m->point_b, &wolf->minimap,
			(wolf->map[(unsigned int)m->ry][(unsigned int)m->rx].win == 1)
			? VERT_C : BLANC);
		}
		m->rx = ++m->rx;
